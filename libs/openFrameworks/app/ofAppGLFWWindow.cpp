...
//	ofLogNotice("ofAppGLFWWindow") << "WINDOW MODE IS " << screenMode;
	...
	glfwWindowHint(GLFW_DECORATED, settings.decorated);
		...
		if(settings.glesVersion>=1){
			currentRenderer = std::shared_ptr<ofBaseRenderer>(new ofGLProgrammableRenderer);
		}else{
			currentRenderer = std::shared_ptr<ofBaseRenderer>(new ofGLRenderer);
		}
	#else
    ...
    if( windowMode == OF_FULLSCREEN){
        ...
        if( bMultiWindowFullscreen ){
			...
			for(int i = 0; i < monitorCount; i++){
				...
				minX = std::min<int>(tempXPos,minX);
				minY = std::min<int>(tempYPos,minY);
				maxX = std::max<int>(maxX,tempXPos + desktopMode->width);
				maxY = std::max<int>(maxY,tempYPos + desktopMode->height);

				xpos = std::min<int>(xpos,tempXPos);
				ypos = std::min<int>(ypos,tempYPos);
			}

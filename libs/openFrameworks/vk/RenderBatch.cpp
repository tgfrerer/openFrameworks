...
const::vk::DescriptorSet of::RenderContext::getDescriptorSet( uint64_t descriptorSetHash, size_t setId, const std::unique_ptr<of::DrawCommand> & drawCommand ){
	...
	for ( const auto & d : descriptors ){
		++requiredPoolSizes[reinterpret_cast<const size_t&>( d.type )];
	}
...
void of::RenderBatch::draw( const std::unique_ptr<of::DrawCommand>& dc ){
	...
	for ( size_t setId = 0; setId != setLayoutKeys.size(); ++setId ){
		...
		dynamicBindingOffsets.insert( dynamicBindingOffsets.end(), dc->getDescriptorSetData( setId ).dynamicBindingOffsets.begin(), dc->getDescriptorSetData( setId ).dynamicBindingOffsets.end() );
